...
function DonatersPage() {
  const ALL_DONATERS = [{name: "a", sum: 101}, {name: "a", sum: 100}, {name: "a", sum: 44}, {name: "a", sum: 100}, {name: "a", sum: 20}, {name: "a", sum: 38}, ]
  const topdonater = ALL_DONATERS.reduce((max, donater) => (
    (+donater.sum > +max.sum) ? donater : max
  ),
  ALL_DONATERS[0]
  )
  const lastdonation = ALL_DONATERS.pop()
  return <div className={styles.donaterspage}>
    ...
    {ALL_DONATERS.map((person, index) => (
      <div className={styles.donater__block} key={index}>
        <span className={styles.donater__item}>{person.name}</span><span  className={styles.donater__item}>{person.sum}</span>
      </div>
    ))}
  </div>;
}
